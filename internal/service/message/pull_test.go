...
func testPullAfterQuit(t *testing.T, st store.Store) {
	...
	writer := New(Adapt(st), NoopPublisher{}, Config{MaxContentBytes: 64})
	...
	reader := New(Adapt(pullInterleavingStore{Store: st, changeMembership: change}), NoopPublisher{}, Config{MaxContentBytes: 64})
